...
namespace asciidia\plugins {
     ...
     */
    class gitgraph extends \asciidia\plugin
    /**/
    {
        /**
    ...
    example: %2\$s -i /path/to/git-repository -o - -r 2011-01-01..2012-01-01 -u week -g commits,sloc\n",
        ...
        /**/
        {
            ...
            if ($types['commits'] || $types['commits_avg']) {
                // render commits
                $chart = new \asciidia\chart(
                    $this->getContext(), 
                    $width, $height, 
                    array('ticks' => $ticks, 'grid' => $grid)
                );

                $set     = new \asciidia\chart\dataset('commits', $data['commits']);
                $set_avg = $set->getSimpleMovingAverage('commits avg');

                $chart->addGraph(new \asciidia\chart\graph\bar($set));
                $chart->addGraph(new \asciidia\chart\graph\spline($set_avg));
                $chart->create();
            }
            ...
            if ($types['files'] || $types['files_avg']) {
                ...
                $set     = new \asciidia\chart\dataset('files', $data['files']);
                $set_avg = $set->getSimpleMovingAverage('files avg');

                $chart->addGraph(new \asciidia\chart\graph\line($set));
                $chart->addGraph(new \asciidia\chart\graph\spline($set_avg));
                $chart->create();
            }
            ...
            if ($types['sloc'] || $types['sloc_avg']) {
                ...
                $set     = new \asciidia\chart\dataset('sloc', $data['sloc']);
                $set_avg = $set->getSimpleMovingAverage('sloc avg');

                $chart->addGraph(new \asciidia\chart\graph\layered(array(
                    new \asciidia\chart\dataset('inserts', $data['inserts']),
                    new \asciidia\chart\dataset('deletes', $data['deletes'])
                ),
                array(
                    array(
                ...
                $chart->addGraph(
                    new \asciidia\chart\graph\line($set),
                    array(
